...
**Current data window:** Feb 16 – Mar 1, 2026 (14 days, 691 final games, 365 unique teams)
...
| Field | Value |
|---|---|
| Trigger | `schedule: cron: '0 16 * * *'` (11 AM ET = 16:00 UTC) + `workflow_dispatch` (manual) |
| Runner | `ubuntu-latest` |
| Python version | `3.9` |
| Steps | checkout → install deps → run `predict_today.py --export-json` → commit `predictions_latest.json` to repo |
...
2. **Table 4 (PRIMARY)** — HIGH/MED Confidence Picks
   - Columns: Conf badge, Pick, Matchup (Away @ Home), Win Prob, Moneyline, Std Dev
...
### 6.6 Data Freshness Flow

```
11:00 AM ET daily:
  GitHub Actions → runs predict_today.py
                 → writes Python scripts/predictions_latest.json
...
|---|---|---|---|
| 1 | **Tiny training window** | 14 days of data (~691 games) is insufficient for reliable generalization; models will overfit to Feb 2026 game patterns | Expand date range to full season (Nov – Mar) |
